 ...
 *
 * 两个关键设计（都是踩过的坑）：
 * 1. 取 14 天窗口而不是刚好 7 天。7 天窗口会横跨两个自然周（实测 14 天窗口横跨 3 周，
 *    分组长度形如 [2,7,6]），且首/末周可能不满 7 天，所以必须把 weeks 摊平后按日期取
 *    末尾 7 个 —— 绝不能按周下标定位。
 * 2. 展示的日期全部取自 API 返回的 date 字段，本机时钟只用来划一个宽松的 from/to 范围。
 *    GitHub 按账号时区切分自然日，而 Actions 跑在 UTC，若拿本地时钟自行推算「今天」
 *    会在时区边界整体错一天；多取 7 天做缓冲即可吸收这点偏差。
 *
 * 独立验证结论：UTC 00:00 / 23:59 / UTC+8 晚间 / UTC-10 晨间四个时刻取到的末尾 7 天
 * 完全一致；14 天窗口恒返回 15 条日记录，故 slice(-7) 恒为 7 天（1990–2030 各时段实测）。
 *
